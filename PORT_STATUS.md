...
| General placement, symmetry, compaction, and bin packing | The even-seed graph-distance initializer, placement cell sizing, geometry constants, box distances, and compass helpers are translated and tested. Annealing, compaction, and bin packing remain unported, so the initializer is not connected to the renderer. |
