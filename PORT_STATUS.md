...
| General placement, symmetry, compaction, and bin packing | The even-seed graph-distance initializer, odd-seed ordinary-node initializer including fixed anchors, placement cell sizing, geometry constants, ordinary-box overlap, box distances, compass helpers, Go-compatible seeded random stream, ordinary-node sizeless optimizer, and sized placement candidate/move search are translated. Ordinary-node sizeless edge cost, sized edge cost, turn cost, and local symmetry now match 43 pinned upstream fixtures. Sized scoring still needs semi-diagonal alternates, labels, crossings, and compound branches. Compaction and bin packing remain unported, so the optimizer is not connected to the renderer. |
