...
export function initializeNodes(graph: TalaGraph): void {
  ...
  const order = reachableBreadthFirst(graph.nodes[0]!);
  if (order.length !== graph.nodes.length) throw new Error('initializeNodes needs one connected component');
  for (const node of graph.nodes) node.topLeft = undefined;
  order[0]!.topLeft = { x: graph.nodes.length, y: graph.nodes.length };
  const occupied = new Set<string>([`${graph.nodes.length},${graph.nodes.length}`]);
  for (const node of order.slice(1)) {
    ...
    const minX = x - distance - 2, maxX = x + distance + 2;
    const minY = y - distance - 2, maxY = y + distance + 2;
